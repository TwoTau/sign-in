...
const DATE_FORMAT_MEETING_KEY = "YYYY-MM-DD"; // iso
...
const CLOUD_FUNCTIONS_URL = "https://....cloudfunctions.net";
...
$(function() {
    ...
    function signInOut(name, signIn) {
        ...
        if(signIn) {
            inOrOut = "in";
            webhookUrl = DISCORD_SIGN_IN_WEBHOOK_URL;
            addToPresentMembersList(name);
        } else {
            inOrOut = "out";
            webhookUrl = DISCORD_SIGN_OUT_WEBHOOK_URL;
            removeFromPresentMembersList(name);
        }

        const nameEncoded = encodeURIComponent(name);
        $.get({
            url: `${CLOUD_FUNCTIONS_URL}/signin?name=${nameEncoded}&type=${inOrOut}`,
        }).fail(error => {
            console.error(error);
        });
    ...
    $("#submit-correction").click(function() {
        ...
        if(correction) {
            const requestedDate = moment($("#date-to-correct").val(), "YYYY-MM-DD").format("YYYY-MM-DD");

            const nameEncoded = encodeURIComponent(nameCorrectionSelected);
            const dateEncoded = encodeURIComponent(requestedDate);
            const requestEncoded = encodeURIComponent(correction);

            $.get({
                url: `${CLOUD_FUNCTIONS_URL}/addcorrection?name=${nameEncoded}&date=${dateEncoded}&request=${requestEncoded}`,
            }).fail(error => {
                console.error(error);
            });
            
            alert(`Your correction "${correction}" has been recorded.`);
            sendDiscordWebhook(DISCORD_CORRECTION_WEBHOOK_URL, `**${nameCorrectionSelected}** submitted correction for ${requestedDate}: "${correction}" at ${moment().format("h:mm A")}.`);
